...
function sendQuestionToAPI(question) { 
    ...
    //generate random response number to question between [1,5]
    ...
    .then(res => res.json())
    .then(data => {
        // Check if we received multiple responses
        if (Array.isArray(data.responses)) {
            // Show multiple responses with slight delays between them
            showMultipleResponses(data.responses);
        } else {
            // Fallback for backward compatibility
            showBotResponse(data.response || data.responses);
        }
